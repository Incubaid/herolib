...
// generate_exec generates all files for an installer module
pub fn generate_exec(args ModuleMeta, reset bool) ! {
	...
	// Generate templates directory if enabled
	...
	if args.templates {
		if !path_templ_dir.exists() {
			mut templ_config := $tmpl('templates/config.yaml')
			pathlib.template_write(templ_config, '${args.path}/templates/config.yaml',
				true)!
		}
	}
