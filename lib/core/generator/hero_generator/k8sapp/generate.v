...
// generate_exec generates all files for a k8sapp module
pub fn generate_exec(args ModuleMeta, reset bool) ! {
	...
	if !path_actions.exists() {
		console.print_debug('write actions')
		mut templ_actions := $tmpl('templates/objname_actions.vtemplate')
		pathlib.template_write(templ_actions, '${args.path}/${args.name}_actions.v', true)!
	}
	...
	// Generate templates directory (always for k8sapp)
	mut path_templ_dir := pathlib.get_dir(path: args.path + '/templates', create: false)!
	if reset {
		path_templ_dir.delete()!
	}
	if !path_templ_dir.exists() {
		mut templ_deployment := $tmpl('templates/deployment.yaml')
		pathlib.template_write(templ_deployment, '${args.path}/templates/deployment.yaml',
			true)!
	}
