...
fn test_directory_operations() {
	...
	// Add a directory
	mut updated_test_dir := fs_factory.fs_dir.get(dir_id)!
	updated_test_dir.directories << root_dir_id
	mut updated_dir_id := fs_factory.fs_dir.set(updated_test_dir)!
	updated_test_dir = fs_factory.fs_dir.get(updated_dir_id)!
	assert updated_dir_id == updated_test_dir.id
	println('✓ Added directory to directories list')

	// Add a file
	updated_test_dir.files << 123
	updated_dir_id = fs_factory.fs_dir.set(updated_test_dir)!
	updated_test_dir = fs_factory.fs_dir.get(updated_dir_id)!
	assert updated_dir_id == updated_test_dir.id
	println('✓ Added file to files list')

	// Add a symlink
	updated_test_dir.symlinks << 456
	updated_dir_id = fs_factory.fs_dir.set(updated_test_dir)!
	updated_test_dir = fs_factory.fs_dir.get(updated_dir_id)!
	assert updated_dir_id == updated_test_dir.id
