...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Setup Delete Collection button
    document.getElementById('deleteCollectionBtn').addEventListener('click', async () => {
        try {
            const currentCollection = collectionSelector.getCurrentCollection();
            if (!currentCollection) {
                window.showNotification('No collection selected', 'warning');
                return;
            }

            const confirmed = await window.ConfirmationManager.confirm(
                `Are you sure you want to delete the collection "${currentCollection}" and all its contents?`,
                'Delete Collection',
                true
            );
            ...
            // Delete the collection directly at base URL level (not through getFullUrl)
            const deleteUrl = `${webdavClient.baseUrl}${currentCollection}/`;
            const response = await fetch(deleteUrl, {
                method: 'DELETE',
                headers: webdavClient.getAuthHeaders()
            });
            ...
            // Clear the last viewed page for this collection
            if (editor && editor.clearLastViewedPage) {
                editor.clearLastViewedPage(currentCollection);
            }
            ...
            if (editor) {
                ...
                editor.previewElement.innerHTML = `
                    <div class="text-muted text-center mt-5">
                        <p>Collection deleted. Select another collection to continue.</p>
                    </div>
                `;
            }
            ...
            // Reload file tree to clear the sidebar
            if (fileTree) {
                await fileTree.load();
            }

            window.showNotification(`Collection "${currentCollection}" deleted`, 'success');
        } catch (error) {
            Logger.error('Failed to delete collection:', error);
            window.showNotification('Failed to delete collection', 'error');
        }
    });
    ...
    // Pass null for filenameInputId since we removed the editor header
    editor = new MarkdownEditor('editor', 'preview', null, !isEditMode);
    ...
    collectionSelector.onChange = async (collection) => {
        try {
            await fileTree.load();
            // In view mode, show collection overview when collection changes
            if (!isEditMode) {
                await showDirectoryPreview(collection);
            }
        } catch (error) {
    ...
    if (urlCollection) {
        ...
        if (urlFilePath) {
            ...
            // Collection-only URL in view mode: show collection overview
            console.log('[URL LOAD] Collection-only URL, showing collection overview');
            await showDirectoryPreview(urlCollection);
        }
    } else if (!isEditMode) {
        // No URL collection specified, in view mode: show empty state
        editor.previewElement.innerHTML = `
            <div class="text-muted text-center mt-5">
    ...
    if (isEditMode) {
        ...
        // New button shows a menu for New File / New Folder
        ...
        newBtn.addEventListener('click', (e) => {
            ...
            newItemMenu.style.left = `${rect.left}px`;
            newItemMenu.style.top = `${rect.bottom + 5}px`;
            newItemMenu.style.display = 'block';
        });

        // Handle new item menu clicks
        newItemMenu.addEventListener('click', async (e) => {
            ...
            // Get the current collection root as the target path
            const currentCollection = collectionSelector.getCurrentCollection();
            if (!currentCollection) {
                window.showNotification('No collection selected', 'warning');
                return;
            ...
            // Execute the action using fileTreeActions
            if (window.fileTreeActions) {
                await window.fileTreeActions.execute(action, currentCollection, true);
            }
        });

        // Close menu when clicking outside
        document.addEventListener('click', (e) => {
            if (!newItemMenu.contains(e.target) && e.target !== newBtn) {
                newItemMenu.style.display = 'none';
            }
        });
        ...
        // Initially hide delete button (no file selected)
        deleteBtn.style.display = 'none';

        // Listen for file-selected event to show/hide delete button
        window.eventBus.on('file-selected', (filePath) => {
            if (filePath) {
