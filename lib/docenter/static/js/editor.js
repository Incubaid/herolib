...
class MarkdownEditor {
    constructor(editorId, previewId, filenameInputId, readOnly = false) {
        ...
        // filenameInput is optional now (removed from UI)
        this.filenameInput = filenameInputId ? document.getElementById(filenameInputId) : null;
    ...
    async loadFile(path) {
        try {
            ...
            // Dispatch event to update delete button visibility
            window.eventBus.dispatch('file-selected', path);
            ...
            // Show 404 message if file not found
            if (error.message && (error.message.includes('404') || error.message.includes('Not Found') || error.message.includes('not found') || error.message.includes('does not exist'))) {
                this.show404Message(path);
            } else {
                // Show generic error message
                this.showErrorMessage('Failed to load file', error.message || 'Unknown error');
    ...
    /**
     * Show 404 message in preview pane
     */
    show404Message(path) {
        const fileName = path.split('/').pop();
        this.previewElement.innerHTML = `
            <div class="alert alert-warning mt-4">
                <h4><i class="bi bi-exclamation-triangle"></i> File Not Found</h4>
                <p>The file <strong>${fileName}</strong> does not exist.</p>
                <p class="mb-0">It may have been deleted or moved.</p>
            </div>
        `;
    }
    ...
    showErrorMessage(title, message) {
        this.previewElement.innerHTML = `
            <div class="alert alert-danger mt-4">
                <h4><i class="bi bi-exclamation-circle"></i> ${title}</h4>
    ...
    /**
     * Clear the last viewed page for a specific collection
     * Used when a collection is deleted
     */
    clearLastViewedPage(collection) {
        const storageKey = `${this.lastViewedStorageKey}:${collection}`;
     ...
     */
    async save() {
        // Use currentFile if available, otherwise try filenameInput (for backwards compatibility)
        let path = this.currentFile;
        if (!path && this.filenameInput) {
            path = this.filenameInput.value.trim();
        }

        if (!path) {
            if (window.showNotification) {
                window.showNotification('No file selected. Please select or create a file first.', 'warning');
            }
            return;
        }
    ...
    newFile() {
        ...
        this.updatePreview();
        // Dispatch event to update delete button visibility
        window.eventBus.dispatch('file-selected', null);
